...
class TASDataset(Dataset):
    def __init__(self, data_folder, eval=False, mode=None, aug_flag=0):
        ...
        self.transform_aug = transforms.Compose([transforms.ToTensor(),\
                                                 transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225)),\
                                                 transforms.CenterCrop(200),\
                                                 transforms.Resize((384, 768)),                                              
                                              ]) 

        self.transform_aug_mask = transforms.Compose([transforms.ToTensor(),\
                                                transforms.CenterCrop(200),\
                                                transforms.Resize((384, 768)),\
                                              ])
    ...
    def __getitem__(self, idx):    
        ...
        if self.aug_flag==1 and random.random()>0.5:
            image = self.transform_aug(image).float()
            mask = self.transform_aug_mask(mask)
